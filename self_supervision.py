...
from dataset import my_collate_fn, PretrainLiverDataset3
from torch.utils.data import DataLoader
from models import FeaExtractor2
...
parser.add_argument("--log_file", type=str, default='log_test.txt', help="batch_size")
...
if 'fea_ex' in opt.sup_model:
    ...
    is_resnet = False
    if opt.sup_model in 'fea_ex2':
        model = FeaExtractor2(in_features=opt.channels, num_features=opt.num_feats, num_classes=opt.num_class, pre_train=True)
    elif opt.sup_model in 'fea_ex5':
        model = FeaExtractor5(in_features=opt.channels, num_features=opt.num_feats, num_classes=opt.num_class, pre_train=True)
    elif opt.sup_model in 'fea_ex6':
        model = FeaExtractor6(in_features=opt.channels, num_features=opt.num_feats, num_classes=opt.num_class, pre_train=True)

    '''
    chkpt_path='./chkpts/fea_ex2_smooth_1_epoch_run_1/checkpoint_best.pth.tar'
    print('loading checkpoint model from:', chkpt_path)
...
for epoch in range(opt.n_epochs):
    ...
    for i, data in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        #if opt.sup_model == 'fea_ex2' or opt.sup_model =='fea_ex5':
        if opt.sup_model == 'fea_ex2' or opt.sup_model =='fea_ex5' or opt.sup_model == 'fea_ex6':
