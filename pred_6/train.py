...
from dataset import LiverDataset, LiverDataset2
import utils
...
def main():
    ...
    if opt.model['use_resnet'] == 1:
        ...
        train_set = LiverDataset2('{:s}/train{:s}.h5'.format(opt.train['data_dir'], fold_num), data_transform)
        test_set = LiverDataset2('{:s}/test{:s}.h5'.format(opt.train['data_dir'], fold_num), data_transform)
    # loading from the prew-trained model
    #print("=> loading pre-trained model from path", opt.model['sup_model_path'])
    #model.set_fea_extractor(opt.model['sup_model_path'])
    # ---------- End Data loading ---------- #
